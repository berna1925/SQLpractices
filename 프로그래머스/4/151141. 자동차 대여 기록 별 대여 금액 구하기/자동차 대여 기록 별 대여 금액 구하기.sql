# 2025-07-29 못 푼 문제
# LEFT JOIN의 사용처 감각을 잘 못 잡고 있다.. 문제에는 LEFT JOIN을 쓴 뒤 NULL을 WHERE로 쳐내는 것도 많다
# 
SELECT 
    H.HISTORY_ID,
    ROUND((1 - IFNULL(P.DISCOUNT_RATE, 0) / 100) * C.DAILY_FEE * (DATEDIFF(H.END_DATE, H.START_DATE) +1), 0) AS FEE
FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY H
JOIN CAR_RENTAL_COMPANY_CAR C ON H.CAR_ID = C.CAR_ID
LEFT JOIN CAR_RENTAL_COMPANY_DISCOUNT_PLAN P 
    ON P.CAR_TYPE = C.CAR_TYPE
    AND ((DATEDIFF(H.END_DATE, H.START_DATE) >= 7 AND DATEDIFF(H.END_DATE, H.START_DATE) < 30 AND P.DURATION_TYPE = '7일 이상')
